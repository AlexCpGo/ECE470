...
_img = cv2.Canny(_xgrd, _ygrd, 30, 250)
...
_xgrd = cv2.Sobel(_img_gray, cv2.CV_16SC1, 1, 0)
_ygrd = cv2.Sobel(_img_gray, cv2.CV_16SC1, 0, 1) 
_img = cv2.Canny(_xgrd, _ygrd, 30, 60)
cv2.imshow('_Canny', _img)


_img, contoursNoFilter, hierarchy = cv2.findContours(_img, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)

for i in range(len(contoursNoFilter))
...
for i in range(len(center_value)):
    ...
    _draw_img = cv2.putText(_draw_img,str(center_value[i][0])+","+str(center_value[i][1]),(center_value[i][0]+10,center_value[i][1]+10),cv2.FONT_HERSHEY_PLAIN, 2,(255,0,255),3)
