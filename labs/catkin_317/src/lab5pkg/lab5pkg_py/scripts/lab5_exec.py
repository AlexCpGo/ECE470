...
def main():
	...
	circstart = 0.094
	rectstart = 0.094
	rectstack=[0.314,-0.126]
	circstack=[0.242,-0.246]
	move_arm(pub_setjoint,lab_invk(0.036,-0.408,0.22,0))
		
	for i in range(len(center_values)):
		move_arm(pub_setjoint,lab_invk(center_values[i][0],center_values[i][1],postsuck_z,theta[i]))
		move_arm(pub_setjoint,lab_invk(center_values[i][0],center_values[i][1],suck_z,theta[i]))
		gripper(pub_setio,suction_on)
		time.sleep(1)
		move_arm(pub_setjoint,lab_invk(center_values[i][0],center_values[i][1],postsuck_z,0))
		
		if shape[i]:
			move_arm(pub_setjoint,lab_invk(circstack[0],circstack[1],circstart,0))
			circstart = circstart +0.004
		else:
			move_arm(pub_setjoint,lab_invk(rectstack[0],rectstack[1],rectstart,0))
			rectstart = rectstart +0.004
